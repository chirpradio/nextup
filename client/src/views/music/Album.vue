    ...
    <div class="px-6" v-if="!loading">
      <div class="row mb-3">
        ...
        <div class="col-md-10">
          <h1>{{album.title}}</h1>
          <h2>
            <span v-if="album.is_compilation" class="badge rounded-pill bg-secondary">Compilation</span>
      ...
      <h3>Tracks</h3>
      <ol class="list-group list-group-numbered list-group-flush col-12 col-md-9">
        <li v-for="track in album.tracks" :key="track.id" class="list-group-item d-flex align-items-start"> 
          <div class="row ms-2 flex-fill">
            <div class="col-12 col-md-9">
              <div class="col mb-1">
                <TrackRecommended :track="track" class="pe-1" />
                <TrackExplicit :track="track" class="pe-1" />
                <span class="fw-bold">{{track.title}}</span><span v-if="album.is_compilation"> by <ArtistLink :artist="track.track_artist" /></span>
              </div>
              <div class="col mb-1">
                <TrackDuration :track="track" /> 
                &middot;
                <small class="text-muted fw-light">{{track.bit_rate_kbps}}kbps</small>
              </div> 
            </div>
            <AddToCrate :keyToAdd="track.__key" :limitWidth="true" class="col-12 col-md-3" />
          </div>
        </li>   
      </ol>
    </div>
...
export default {
  components: { AddToCrate, RecordSpinner, ArtistLink, TagList, AlbumArt, TrackDuration, TrackTitle, TrackRecommended, TrackExplicit, },
  data () {
    return {
      loading: true,
    }
