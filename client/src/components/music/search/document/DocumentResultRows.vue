  ...
  <div>
    <div v-for="hit in results.hits" :key="hit._id" class="row border-top py-2 mx-0">
      ...
      <div v-if="hit.highlight" class="col-sm-5 search_result__col">
        <ul v-if="hit.highlight['unsafe_text.normalized']" class="list-unstyled">        
          <li v-for="fragment in hit.highlight['unsafe_text.normalized']" :key="fragment" class="mb-1 text-muted whitespace-initial"><span v-html="addEllipses(fragment)"></span></li>        
        </ul>
      </div>
    </div> 
  </div>
