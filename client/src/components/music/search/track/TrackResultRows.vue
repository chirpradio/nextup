  ...
  <div>
    <div v-for="hit in results.hits" :key="hit._id" class="row border-top py-2 mx-0">
      ...
      <div class="col-sm-3 search_result__col">
        <div v-if="hit._source.album.is_compilation && hit._source.track_artist">{{hit._source.track_artist.name}}</div>
        <ArtistLink v-if="!hit._source.album.is_compilation" :artist="hit._source.album.album_artist" />                 
      </div>
      <div class="col-sm-3 search_result__col">        
...
export default {
  ...
  components: { AlbumArtLink, AlbumTitleLink, ArtistLink, TagList, TrackDuration, TrackTitle },
  props: {
    results: {
      type: Object,
      default: {
